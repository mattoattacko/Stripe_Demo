...
async function decodeJWT(req: Request, res: Response, next: NextFunction) {
  //checks to see if the header has the Bearer token
  //if it does, splits ID token off of value
  if (req.headers?.authorization?.startsWith('Bearer ')) {
    const idToken = req.headers.authorization.split('Bearer ')[1];

    try {
      const decodedToken = await auth.verifyIdToken(idToken);
      //result is current users record from firebase
 ...
 */
//purpose is to catch any errors that happen within our async code. eg: send request to Stripe but they send back an error.
//Usually Express cant catch these errors and API will hang then timeout
...
app.get(
  ...
  runAsync(async (req: Request, res: Response) => {
    ...
    // we want to send back an array of their payment methods, fount on the 'wallet.data' property 
...
// Unsubscribe or cancel a subscription (ie: updates subscription)
app.patch(
  '/subscriptions/:id',
  runAsync(async (req: Request, res: Response) => {
    const user = validateUser(req);
