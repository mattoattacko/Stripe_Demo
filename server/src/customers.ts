...
export async function getOrCreateCustomer(userId: string, params?: Stripe.CustomerCreateParams) {
    ...
    //retrieve users firestore doc checks to see if a customer ID already exists there
    ...
    if (!stripeCustomerId) {
        ...
        //if the customerID does exist, we retrieve that customers record from Stripe with the corresponding stripe customer ID
